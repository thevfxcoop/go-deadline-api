...
// GetTaskReportContents returns task reports with "all contents"
func (this *Client) GetTaskReportContents(job string, task uint) ([]*schema.TaskReport, error) {
	var objs []map[string]interface{}
	payload := NewGetPayload(ContentTypeJson)
	if err := this.Do(payload, &objs, OptPath("api/taskreports"), optJobId(job), optTaskId(task), optData("allcontents")); err != nil {
		return nil, err
	}
